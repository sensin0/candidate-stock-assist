import fs from "node:fs";
import path from "node:path";
import { spawnSync } from "node:child_process";
import { fileURLToPath } from "node:url";
import { pythonExecutable } from "./python-runtime.mjs";

const rootDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

const requiredFiles = [
  "package.json",
  "README.md",
  "STOCK_MASTER_COLUMNS.md",
  "PRODUCTION_CHECKLIST.md",
  "OPERATIONS_RUNBOOK.md",
  ".gitignore",
  ".github/workflows/morning-pages.yml",
  "app/index.html",
  "app/app.js",
  "app/styles.css",
  "app/generated-research.js",
  "app/generated-expansion-preview.js",
  "app/generated-promotion-readiness.js",
  "app/generated-hidden-gems.js",
  "app/generated-hidden-gems-draft.js",
  "app/generated-financial-confirmation.js",
  "app/generated-financial-screening.js",
  "scripts/build-auto-financial-followup.mjs",
  "scripts/update-data.mjs",
  "scripts/audit-csv-store.mjs",
  "scripts/build_sqlite_store.py",
  "scripts/run-sqlite-store.mjs",
  "scripts/runtime-stock-data.mjs",
  "scripts/backtest-core.mjs",
  "scripts/backtest-timing.mjs",
  "scripts/monthly_signal_backtest.py",
  "scripts/research-universe-price-backtest.mjs",
  "scripts/analyze-multibagger-candidates.mjs",
  "scripts/build-promotion-candidates.mjs",
  "scripts/build-promotion-readiness.mjs",
  "scripts/build-hidden-gems.mjs",
  "scripts/build-stock-master-draft-from-hidden-gems.mjs",
  "scripts/build-financial-confirmation-queue.mjs",
  "scripts/build-financial-confirmation-worklist.mjs",
  "scripts/enrich-financial-worklist.mjs",
  "scripts/screen-financial-worklist.mjs",
  "scripts/apply-financial-worklist.mjs",
  "scripts/promote-confirmed-candidates.mjs",
  "scripts/build-production-next-steps.mjs",
  "scripts/build-research-data.mjs",
  "scripts/build-stock-master-draft-from-promotions.mjs",
  "scripts/build-stock-master-expanded-preview.mjs",
  "scripts/research-morning.mjs",
  "scripts/build-stock-master-from-input.mjs",
  "scripts/production-check.mjs",
  "scripts/build-pages.mjs",
  "scripts/generate-morning-report.mjs",
  "scripts/check-new-buy-candidates.mjs",
  "scripts/csv-utils.mjs",
  "scripts/csv-utils.test.mjs",
  "scripts/stock-master.test.mjs",
  "scripts/stock-master-draft.test.mjs",
  "scripts/stock-master-expanded-preview.test.mjs",
  "scripts/promotion-readiness.test.mjs",
  "scripts/hidden-gems.test.mjs",
  "scripts/hidden-gems-draft.test.mjs",
  "scripts/financial-confirmation.test.mjs",
  "scripts/financial-confirmation-worklist.test.mjs",
  "scripts/enrich-financial-worklist.mjs",
  "scripts/screen-financial-worklist.mjs",
  "scripts/apply-financial-worklist.test.mjs",
  "scripts/promote-confirmed-candidates.test.mjs",
  "scripts/auto-financial-followup.test.mjs",
  "scripts/universe-metrics.test.mjs",
  "scripts/production-next-steps.test.mjs",
  "scripts/build-stock-master-from-input.test.mjs",
  "scripts/confirm-stock.mjs",
  "scripts/confirm-stock.test.mjs",
  "scripts/privacy-check.mjs",
  "scripts/universe-check.mjs",
  "scripts/build-universe-check-status.mjs",
  "scripts/build-universe-buy-candidates.mjs",
  "scripts/review-universe-buy-candidates.mjs",
  "scripts/build-price-validation-queue.mjs",
  "scripts/build-universe-metrics.mjs",
  "scripts/complete-universe-data.mjs",
  "scripts/universe-screen.mjs",
  "scripts/production-check.test.mjs",
  "scripts/data-quality.test.mjs",
  "scripts/sheet-templates.test.mjs",
  "scripts/notify-discord.test.mjs",
  "scripts/check-new-buy-candidates.test.mjs",
  "scripts/chart-data-ready.test.mjs",
  "GOOGLE_SHEETS_SETUP.md",
  "sheet-templates/stock-master.csv",
  "sheet-templates/stock-master-input.csv",
  "sheet-templates/price-updates.csv",
  "sheet-templates/disclosures.csv",
  "sheet-templates/edinet-facts.csv",
  "sheet-templates/watchlist.csv",
  "sheet-templates/listed-universe.csv",
  "sheet-templates/universe-metrics.csv",
  "data/stock-master.csv",
  "data/listed-universe.csv",
  "data/universe-metrics.csv",
  "data/universe-completion-overrides.csv",
  "data/universe-check-status.csv",
  "data/universe-buy-candidates.csv",
  "data/universe-buy-candidate-review.csv",
  "data/stock-master-universe-promotion-draft.csv",
  "data/stock-master-input.csv",
  "data/price-updates.csv",
  "data/price-validation-queue.csv",
  "data/disclosures.csv",
  "data/edinet-facts.csv",
  "data/watchlist.csv",
  "scripts/providers/watchlist-provider.mjs",
  "scripts/providers/earnings-provider.mjs",
  "scripts/providers/backtest-provider.mjs",
  "data/backtest-results.csv",
  "data/earnings-facts.csv",
  "sheet-templates/backtest-results.csv",
  "sheet-templates/earnings-facts.csv",
  "data/universe-price-backtest.csv",
  "data/monthly-price-history.csv",
  "data/monthly-signal-backtest.csv",
  "data/monthly-signal-summary.csv",
  "data/multibagger-candidates.csv",
  "data/promotion-candidates.csv",
  "data/promotion-readiness.csv",
  "data/hidden-gems.csv",
  "data/stock-master-hidden-gems-draft.csv",
  "data/financial-confirmation-queue.csv",
  "data/financial-confirmation-worklist.csv",
  "data/financial-confirmed-input.csv",
  "data/stock-master-promoted.csv",
  "data/auto-financial-followup.csv",
  "data/stock-master-input-draft.csv",
  "data/stock-master-expanded-preview.csv",
  "data/csv-store-manifest.json",
  "data/sqlite-store-manifest.json",
  "reports/latest-universe-price-backtest.md",
  "reports/latest-monthly-signal-backtest.md",
  "reports/latest-multibagger-candidates.md",
  "reports/latest-multibagger-thresholds.md",
  "reports/latest-exit-target-6276.md",
  "reports/latest-promotion-candidates.md",
  "reports/latest-promotion-readiness.md",
  "reports/latest-hidden-gems.md",
  "reports/latest-hidden-gems-stock-master-draft.md",
  "reports/latest-financial-confirmation.md",
  "reports/latest-financial-confirmation-worklist.md",
  "reports/latest-financial-worklist-enrichment.md",
  "reports/latest-financial-worklist-screening.md",
  "reports/latest-financial-confirmed-input.md",
  "reports/latest-promoted-candidates.md",
  "reports/latest-auto-financial-followup.md",
  "reports/latest-universe-financial-coverage.md",
  "reports/latest-universe-data-completion.md",
  "reports/latest-universe-check-status.md",
  "reports/latest-universe-buy-candidates.md",
  "reports/latest-universe-buy-candidate-review.md",
  "reports/latest-price-validation-queue.md",
  "reports/latest-production-next-steps.md",
  "reports/latest-stock-master-draft.md",
  "reports/latest-stock-master-expanded-preview.md",
  "reports/latest-csv-store-audit.md",
  "reports/latest-sqlite-store.md",
];

const missing = requiredFiles.filter((file) => !fs.existsSync(path.join(rootDir, file)));
if (missing.length) {
  console.error(`不足ファイル: ${missing.join(", ")}`);
  process.exit(1);
}

const checks = [
  ["csv parser", process.execPath, ["scripts/csv-utils.test.mjs"]],
  ["stock master", process.execPath, ["scripts/stock-master.test.mjs"]],
  ["stock master draft", process.execPath, ["scripts/stock-master-draft.test.mjs"]],
  ["stock master expanded preview", process.execPath, ["scripts/stock-master-expanded-preview.test.mjs"]],
  ["promotion readiness", process.execPath, ["scripts/promotion-readiness.test.mjs"]],
  ["hidden gems", process.execPath, ["scripts/hidden-gems.test.mjs"]],
  ["hidden gems draft", process.execPath, ["scripts/hidden-gems-draft.test.mjs"]],
  ["financial confirmation", process.execPath, ["scripts/financial-confirmation.test.mjs"]],
  ["financial confirmation worklist", process.execPath, ["scripts/financial-confirmation-worklist.test.mjs"]],
  ["apply financial worklist", process.execPath, ["scripts/apply-financial-worklist.test.mjs"]],
  ["confirmed promotion", process.execPath, ["scripts/promote-confirmed-candidates.test.mjs"]],
  ["auto financial followup", process.execPath, ["scripts/auto-financial-followup.test.mjs"]],
  ["universe metrics", process.execPath, ["scripts/universe-metrics.test.mjs"]],
  ["production next steps", process.execPath, ["scripts/production-next-steps.test.mjs"]],
  ["stock master builder", process.execPath, ["scripts/build-stock-master-from-input.test.mjs"]],
  ["manual confirm helper", process.execPath, ["scripts/confirm-stock.test.mjs"]],
  ["privacy guard", process.execPath, ["scripts/privacy-check.mjs"]],
  ["csv store audit", process.execPath, ["scripts/audit-csv-store.mjs"]],
  ["sqlite store build", process.execPath, ["scripts/run-sqlite-store.mjs"]],
  ["runtime stock data syntax", process.execPath, ["--check", "scripts/runtime-stock-data.mjs"]],
  ["listed universe", process.execPath, ["scripts/universe-check.mjs"]],
  ["universe screening", process.execPath, ["scripts/universe-screen.mjs"]],
  ["universe buy candidates syntax", process.execPath, ["--check", "scripts/build-universe-buy-candidates.mjs"]],
  ["universe buy candidate review syntax", process.execPath, ["--check", "scripts/review-universe-buy-candidates.mjs"]],
  ["price validation queue syntax", process.execPath, ["--check", "scripts/build-price-validation-queue.mjs"]],
  ["universe data completion syntax", process.execPath, ["--check", "scripts/complete-universe-data.mjs"]],
  ["backtest timing", process.execPath, ["scripts/backtest-timing.mjs"]],
  ["monthly signal backtest syntax", pythonExecutable(), ["-m", "py_compile", "scripts/monthly_signal_backtest.py"]],
  ["promotion candidates syntax", process.execPath, ["--check", "scripts/build-promotion-candidates.mjs"]],
  ["promotion readiness syntax", process.execPath, ["--check", "scripts/build-promotion-readiness.mjs"]],
  ["hidden gems syntax", process.execPath, ["--check", "scripts/build-hidden-gems.mjs"]],
  ["hidden gems draft syntax", process.execPath, ["--check", "scripts/build-stock-master-draft-from-hidden-gems.mjs"]],
  ["financial confirmation syntax", process.execPath, ["--check", "scripts/build-financial-confirmation-queue.mjs"]],
  ["financial confirmation worklist syntax", process.execPath, ["--check", "scripts/build-financial-confirmation-worklist.mjs"]],
  ["financial worklist enrichment syntax", process.execPath, ["--check", "scripts/enrich-financial-worklist.mjs"]],
  ["financial worklist screening syntax", process.execPath, ["--check", "scripts/screen-financial-worklist.mjs"]],
  ["apply financial worklist syntax", process.execPath, ["--check", "scripts/apply-financial-worklist.mjs"]],
  ["confirmed promotion syntax", process.execPath, ["--check", "scripts/promote-confirmed-candidates.mjs"]],
  ["auto financial followup syntax", process.execPath, ["--check", "scripts/build-auto-financial-followup.mjs"]],
  ["production next steps syntax", process.execPath, ["--check", "scripts/build-production-next-steps.mjs"]],
  ["research data syntax", process.execPath, ["--check", "scripts/build-research-data.mjs"]],
  ["stock master draft syntax", process.execPath, ["--check", "scripts/build-stock-master-draft-from-promotions.mjs"]],
  ["stock master expanded preview syntax", process.execPath, ["--check", "scripts/build-stock-master-expanded-preview.mjs"]],
  ["morning research syntax", process.execPath, ["--check", "scripts/research-morning.mjs"]],
  ["sheet templates", process.execPath, ["scripts/sheet-templates.test.mjs"]],
  ["data quality", process.execPath, ["scripts/data-quality.test.mjs"]],
  ["discord notification preview", process.execPath, ["scripts/notify-discord.test.mjs"]],
  ["new buy notification gate", process.execPath, ["scripts/check-new-buy-candidates.test.mjs"]],
  ["chart data ready", process.execPath, ["scripts/chart-data-ready.test.mjs"]],
  ["production readiness", process.execPath, ["scripts/production-check.test.mjs"]],
  ["pipeline", process.execPath, ["scripts/pipeline.mjs"]],
  ["pages build", process.execPath, ["scripts/build-pages.mjs"]],
];

for (const [label, command, args] of checks) {
  const result = runWithRetry(label, command, args);
  if (result !== 0) {
    console.error(`${label} に失敗しました`);
    process.exit(result ?? 1);
  }
}

console.log("GitHub Pages公開準備OK");

function runWithRetry(label, command, args, attempts = 3) {
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    const result = spawnSync(command, args, { cwd: rootDir, stdio: "inherit", shell: false });
    if (result.status === 0) return 0;
    if (attempt < attempts) {
      console.warn(`${label} を再試行します (${attempt + 1}/${attempts})`);
      sleep(750 * attempt);
    } else {
      return result.status ?? 1;
    }
  }
  return 1;
}

function sleep(ms) {
  Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, ms);
}

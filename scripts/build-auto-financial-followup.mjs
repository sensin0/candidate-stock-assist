...
function actionLabel({ score, buyDistance, weakBacktest, noBacktest, backtest }) {
  ...
  if (score >= 75 && buyDistance <= 1.08) return "自動確認済み・買い場接近";
...
function checkItems({ stock, netCashRatio, pbr, per, buyDistance, weakBacktest, noBacktest, backtest }) {
  ...
  if (buyDistance <= 1.08) items.push("買いライン付近なので急騰を追いかけない");
...
function buildReport(rows) {
  const priority = rows.filter((row) => row.action === "自動確認済み・買い場接近" || row.action === "財務確認を進める");

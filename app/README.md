...
このコマンドは `app/sample-data.csv` を基礎データとして読み、株価、開示、EDINET相当の更新データを重ねて、画面が読む `app/generated-data.js` と更新レポート `app/latest-update-report.md` を生成します。将来はこの入力部分を株価API、EDINET、TDnetに差し替えます。

株価だけは先に差し替え口を用意しています。標準では `data/price-updates.csv` を読みます。
...
`sample-data.csv` と同じ列名で取り込めます。金額項目は百万円、株数は株数、株価は円です。
